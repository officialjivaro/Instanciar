...
class LogicInstanceManager(QObject):
    ...
    def _normalize_identity_schema(self, inst: Dict) -> bool:
        ...
        region_profile = normalize_region_profile_key(identity.get("region_profile"))
        if identity.get("region_profile", "") != region_profile:
            identity["region_profile"] = region_profile
            changed = True

        launch_mode = normalize_launch_mode(identity.get("launch_mode"))
        if identity.get("launch_mode") != launch_mode:
            identity["launch_mode"] = launch_mode
            changed = True

        for key in ("region_timezone_auto", "region_language_auto"):
            if not isinstance(identity.get(key), bool):
                identity[key] = False
                changed = True

        resolved = resolved_identity_settings(identity)
        for key in ("timezone", "language", "launch_mode", "region_profile"):
            if identity.get(key) != resolved.get(key):
    ...
    def load(self):
        ...
        for inst in self.data:
            if not inst.get("group"):
                ...
                changed = True
            if "antidetect" in inst:
                inst.pop("antidetect", None)
                changed = True
    ...
    def save_instance_extended(
        ...
        region_profile="",
        launch_mode="normal",
        region_timezone_auto=False,
        region_language_auto=False,
    ):
        ...
        identity["region_profile"] = normalize_region_profile_key(region_profile)
        identity["launch_mode"] = normalize_launch_mode(launch_mode, default="normal")
        identity["region_timezone_auto"] = bool(region_timezone_auto)
        identity["region_language_auto"] = bool(region_language_auto)

        resolved_identity = resolved_identity_settings(identity, default_launch_mode="normal")
        identity["timezone"] = resolved_identity.get("timezone", timezone)
        identity["language"] = resolved_identity.get("language", language)
