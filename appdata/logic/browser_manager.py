...
from appdata.logic.region_profile_logic import resolve_identity_settings
...
class BrowserManager:
    ...
    def _add_profile_health_warnings(self, launch_result: Dict, instance: Dict) -> None:
        """Add non-blocking profile consistency warnings to the launch result."""
        health = build_browser_profile_health(instance)
        for warning in health.get("warnings", []):
            text = str(warning).strip()
            if text and text not in launch_result["warnings"]:
                launch_result["warnings"].append(text)
    ...
    def launch(self, instance: Dict):
        ...
        raw_identity_settings = instance.get("identity") or {}
        identity_settings = resolve_identity_settings(raw_identity_settings)
        instance = dict(instance)
        instance["identity"] = identity_settings

        launch_result = self._new_launch_result()
        self._add_profile_health_warnings(launch_result, instance)
        ...
        options = build_chrome_options(
            ...
            instance.get("proxy"),
            identity_bundle,
            launch_mode=identity_settings.get("launch_mode"),
        )
