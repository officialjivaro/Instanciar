...
from appdata.logic.browser_profile_health import build_browser_profile_health, format_health_warnings
from appdata.logic.region_profile_logic import get_launch_mode_label, get_region_label
...
class MainWindowLogic(QObject):
    ...
    def _instance_issue_count(self, inst):
        try:
            return len(build_browser_profile_health(inst).get("warnings", []) or [])
        except Exception:
            return 0

    def _instance_has_issues(self, inst):
        return self._instance_issue_count(inst) > 0

    def _instance_matches_query(self, inst, group_name, query):
        if not query:
            return True
        ...
        haystack_parts.extend(
            [
                get_region_label(identity.get("region_profile")),
                get_launch_mode_label(identity.get("launch_mode")),
                str(identity.get("timezone") or ""),
                str(identity.get("language") or ""),
            ]
        )

        proxy_info = self._proxy_display(inst)
        haystack_parts.extend([proxy_info.get("status_text", ""), proxy_info.get("reason", "")])
    ...
    def get_dashboard_summary(self):
        ...
        groups = len(self.group_logic.get_all_groups())
        instances = len(self.manager.data)
        issues = sum(1 for inst in self.manager.data if self._instance_has_issues(inst))
        return {"groups": groups, "instances": instances, "issues": issues}

    def refresh_tree(self, tree, search_text="", issues_only=False):
        self._tree = tree
        tree.clear()
        self.group_logic.load_groups()
        query = str(search_text or "").strip()
        issues_only = bool(issues_only)

        for group_name in self.group_logic.get_all_groups():
            group_matches = bool(query and query.casefold() in str(group_name).casefold())
            group_instances = [
                inst for inst in self.manager.data
                if inst.get("group", "Unassigned") == group_name
            ]

            visible_instances = []
            for inst in group_instances:
                if issues_only and not self._instance_has_issues(inst):
                    continue
                if query and not (group_matches or self._instance_matches_query(inst, group_name, query)):
                    continue
                visible_instances.append(inst)

            if query or issues_only:
                if not visible_instances and not (group_matches and not issues_only):
                    continue

            group_item = self._make_group_item(tree, group_name)
            for inst in visible_instances if (query or issues_only) else group_instances:
                proxy_info = self._proxy_display(inst)
                issue_count = self._instance_issue_count(inst)
                suffix = f"  • {issue_count} issue{'s' if issue_count != 1 else ''}" if issue_count else ""
                item = QTreeWidgetItem([" " + inst["name"] + suffix, proxy_info["indicator"]])
        ...
        if tree.topLevelItemCount() == 0:
            empty = QTreeWidgetItem(["No matching instances", ""])
            empty.setData(0, Qt.UserRole, {"type": "empty", "name": ""})
            empty.setFlags(empty.flags() & ~Qt.ItemIsSelectable)
            empty.setForeground(0, QColor(TEXT_COLOR))
    ...
    def get_selection_details(self, tree):
        ...
        if info.get("type") == "instance":
            ...
            identity = inst.get("identity") or {}
            health = build_browser_profile_health(inst)
            health_warnings = health.get("warnings", [])

            rows = [
                ("Group", f"{group_name} (default)" if self._is_reserved_group(group_name) else group_name),
                ("Region Profile", get_region_label(identity.get("region_profile"))),
                ("Browser Mode", get_launch_mode_label(identity.get("launch_mode"))),
                ("Setup Health", health.get("summary", "—")),
                ("Proxy", proxy_text),
            ...
            if health_warnings:
                rows.insert(4, ("Setup Warnings", format_health_warnings(health_warnings)))

            if proxy_info["reason"]:
                rows.insert(7 if health_warnings else 6, ("Proxy Note", proxy_info["reason"]))
    ...
    def _handle_launch_result(self, name, future: Future):
        ...
        if warning_summary or not launch_result["privacy_ok"]:
            if not warning_summary:
                warning_summary = "one or more setup checks"

            # Use the existing info toast kind so this patch stays compatible
            # with the current toast system.
            self.toastRequested.emit(
                f'Browser started for "{name}" with setup warnings: {warning_summary}.',
                "info",
            )
            return
    ...
    def open_commands(self):
        webbrowser.open("https://www.jivaro.net/content/programs/instanciar")
